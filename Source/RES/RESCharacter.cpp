...
void ARESCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ARESCharacter::Move(const FInputActionValue& Value)
{
	FVector2D MovementVector = Value.Get<FVector2D>();

	if (Controller != nullptr && FollowCamera != nullptr)
	{
		// Get camera forward direction (ignoring vertical tilt)
		FVector CameraForward = FollowCamera->GetForwardVector();
		CameraForward.Z = 0;  // Remove vertical component to keep movement flat
		...
		// Move character based on camera-aligned input
		AddMovementInput(CameraForward, MovementVector.Y);
		AddMovementInput(CameraRight, MovementVector.X);
	}
}
 
void ARESCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
